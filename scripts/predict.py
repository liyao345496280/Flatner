...
def predict(input_seq, max_length=128):
    ...
    for name in res:
        ...
        if name == 2:
            for j in res[name]:
                POF.append(input_seq[j[0]:(j[0]+j[1])])
        if name == 3:
            for k in res[name]:
                REI.append(input_seq[k[0]:(k[0]+k[1])])
        if name == 4:
            for k in res[name]:
                CON.append(input_seq[k[0]:(k[0]+k[1])])
        if name == 5:
            for k in res[name]:
                NOA.append(input_seq[k[0]:(k[0]+k[1])])
        if name == 6:
            for k in res[name]:
                ORN.append(input_seq[k[0]:(k[0]+k[1])])
        if name == 7:
            for k in res[name]:
                TIM.append(input_seq[k[0]:(k[0]+k[1])])
